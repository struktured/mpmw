...
(* Creates a new responder given a context, address, serializers, initial state and callback. Tesponder is started as a detached Lwt
 * thread. It will invoke the callback with the initial state and initial request, and will loop continually but with each successive
